...
void dImage::sclaeRender_Center(int destX, int destY, float sizeX, float sizeY, float opacity)
{
	D2D1_RECT_F destRect = dRectMake(destX - _imageInfo->width/2 , destY - _imageInfo->height/2, _imageInfo->width * sizeX, _imageInfo->height* sizeY);

	//rendertarget을 불러와서 이미지의 비트맵을 그려준다
	DTDMANAGER->getBitRenderTarget()->DrawBitmap(_imageInfo->bitMap, destRect, opacity);
}

void dImage::RenderCenter(int destX, int destY, float opacity)
{
	D2D1_RECT_F destRect = dRectMake(destX-_imageInfo->width/2, destY-_imageInfo->height/2, _imageInfo->width, _imageInfo->height);
...
void dImage::aniRender(int destX, int destY, float angle, animation * ani)
{
	...
	render(destX, destY, ani->getFramePos().x, ani->getFramePos().y, ani->getFrameWidth(), ani->getFrameHeight());
	DTDMANAGER->getBitRenderTarget()->SetTransform(Matrix3x2F::Rotation(angle, pt));
