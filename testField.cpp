...
HRESULT testField::init()
{
	...
	_tile->init("tile_IceBoss.map");
	// ´øÀü ¸Ê¿¡¼­´Â ¾Æ·¡²¨ ÁÖ¼® Ç®¸é ´ð´Ï´Ù.
	//_tile->setRandomDungeon("tile_Ice.map");
	_tile->setBossDungeon("tile_Ice.map");
	//CAMERAMANAGER->setBackScreenSize(3840, 2160);
	_player = new player;
	_player->init();
	_player->getPlayerData()->x = 785;
	_player->getPlayerData()->y = 1370;
	...
	_enemyManager->setIceBoss(PointMake( 785, 500));
...
void testField::skillEnemyCollision()
{
	if (!_vEnemy->empty())
	{
...
void testField::skillPlayerCollision()
{
